...
def check_password(message):
    ...
    if password == "12345":
        ...
        bot.register_next_step_handler(message, get_lesson_name)
    else:
        bot.send_message(message.chat.id, "Access denied! Incorrect password.")

def get_lesson_name(message):
    lesson = message.text.strip()
    bot.send_message(message.chat.id, "Please enter the group:")
    bot.register_next_step_handler(message, get_group, lesson)

def get_group(message, lesson):
    group = message.text.strip()
    bot.send_message(message.chat.id, "Please enter the day:")   
    bot.register_next_step_handler(message, get_day, lesson, group)

def get_day(message, lesson, group):
    day = message.text.strip()
    bot.send_message(message.chat.id, "Please enter the timetable:")
    bot.register_next_step_handler(message, get_timetable, lesson, group, day)

def get_timetable(message, lesson, group, day):
    timetable = message.text.strip()
    bot.send_message(message.chat.id, "Please enter the name:")
    bot.register_next_step_handler(message, get_name, lesson, group, day, timetable)

def get_name(message, lesson, group, day, timetable):
